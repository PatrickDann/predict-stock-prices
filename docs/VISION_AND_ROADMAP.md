...
- ✅ **Live updates (SSE) + Redis caching**: a `/api/stream` **Server-Sent Events** endpoint pushes the GDELT news feed as fresh articles land and ticks the browser to refresh the symbol panels — the dashboard **auto-refreshes with no reload** (native `EventSource` auto-reconnect + a ● LIVE indicator). An optional **Redis cache** (`market_intel/cache.py`, cache-aside) fronts the read endpoints (prices/indicators/macro/news/filings) so page loads + every SSE client share one short-lived result instead of hammering Postgres; it **degrades gracefully** to a no-op when Redis is absent/unreachable (startup ping) or errors at runtime (treated as a miss / corrupted-entry → miss), so the dashboard never breaks. The SSE stream polls the DB and emits only on change (worker→API pub/sub fan-out is a later upgrade). Knobs: `CACHE_ENABLED` / `CACHE_TTL_SECONDS` / `STREAM_POLL_SECONDS`. Tests cover the cache (incl. graceful fallback + corrupted-payload→miss + `ttl<=0` disable), the SSE generator, and endpoint caching. _Plan: `docs/plans/live-updates.md`._
