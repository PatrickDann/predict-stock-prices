...
- ✅ **Macro (DBnomics) ingestor** — the *global* macro source (World Bank/IMF/OECD/Eurostat/AMECO, ~80 providers; free, keyless). Reuses the shared `ingest_macro_frame` tail, mirroring FRED: fetch one `PROVIDER/DATASET/SERIES` → parse (`period_start_day` ISO dates, drops `"NA"` markers, guards misaligned arrays) → validate → idempotent upsert into `macro_series` (`source="DBnomics"`). Daily scheduled job (`dbnomics-daily`); one-shot CLI `python src/ingest_dbnomics.py "<id>"`; `--dbnomics` on the worker. `series_id` widened to `String(128)` for the longer triples. (See [docs/plans/dbnomics-macro-ingestor.md](plans/dbnomics-macro-ingestor.md).)
...
- ⬜ Optional extras: curated FreshRSS; range-partitioning (BRIN now → `pg_partman`+`pg_cron` when large) — a scaling optimization, deferred until tables are big.
- _Tests: 101 passing (FRED + DBnomics macro parse/ingest incl. `"NA"` drop, `period_start_day` preference, empty-series no-op, idempotency; EDGAR idempotency/ragged-array edge cases; live-yfinance normalization — hardened via adversarial review); ruff/black clean._

**Phase 1 is functionally complete** — market (CSV + live), macro (FRED + global DBnomics), global news (GDELT) + keyword/semantic search, and filings (EDGAR) all ingest idempotently on a schedule into Postgres with validation gates. Remaining items are optional/scaling.
