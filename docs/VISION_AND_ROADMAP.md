...
- ✅ **Sentiment on the news feed**: each headline scored for financial sentiment and **persisted** on `news_articles.sentiment` (so Phase 3 can aggregate/lag it). Pluggable `SentimentAnalyzer` with a dependency-free, finance-aware **Loughran–McDonald-inspired lexicon** baseline (`market_intel/sentiment.py`) — scored in the GDELT job alongside embeddings (`search.score_sentiment_pending`), surfaced via `/api/news/recent` + `/api/news/search` (`sentiment` + `sentiment_label`), and shown in the feed as a colored ▲/▼/● badge plus a per-feed positive/negative tally. **FinBERT is the documented Phase-3 drop-in** behind the same interface (mirrors the `HashingEmbedder` → `sentence-transformers` upgrade path). Tests cover the analyzer (polarity, negation, bounds, neutral band), the NULL-only idempotent backfill, and the API fields.
